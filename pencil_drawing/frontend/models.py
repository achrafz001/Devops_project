...
with col2:               # To display brand logo
    st.image('devops.png',  width=150)

#Add a header and expander in side bar
st.sidebar.markdown('<p class="font">Photo Converter Service</p>', unsafe_allow_html=True)
with st.sidebar.expander("About the Service"):
     st.write("""
        Use this simple service to convert your favorite photo to a pencil sketch, a grayscale image or an image with blurring effect.  \n
     """)
...
with st.sidebar.form(key='columns_in_form',clear_on_submit=True): #set clear_on_submit=True so that the form will be reset/cleared once it's submitted
    rating=st.slider("Please rate the service", min_value=1, max_value=5, value=3,help='Drag the slider to rate the service. This is a 1-5 rating scale where 5 is the highest rating')
